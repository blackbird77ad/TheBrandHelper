...
const TIERS = [
  {
    ...
    note:   'No logins, no payments, no database — purely beautiful frontend. Payment and backend features are available as extras (see next step).',
    color:  'green',
  },
  {
    key:    'intermediate',
    emoji:  '🚀',
    label:  'Intermediate Website',
    range:  [450, 650],
    ...
    note:   'Stripe or other payment setup requires client to create their own account — we connect it at no extra charge.',
    color:  'blue',
  },
  {
    key:    'complex',
    ...
    note:   'All features available. We build it, you own it.',
    color:  'purple',
  },
];

/* ─── FEATURES PER TIER ──────────────────────────────────────────── */
// Each feature has: which tiers it's "within range" for vs "extra (exceeds range)"
const FEATURES = {
  simple: {
    included: [
    ...
    addons: [
      { key: 'seo',      label: '🔍 Google visibility (SEO)', sub: 'Help people find you on Google',          price: 60,
        explain: 'When someone searches "hair salon in Accra" on Google, SEO is what helps your site show up. Without it, your site may be live but invisible in search results.' },
      { key: 'domain',   label: '🏷 Domain + hosting setup',  sub: 'yourname.com — SSL secured',              price: 50,
        explain: 'Your website address on the internet (e.g. mybusiness.com) plus the server that hosts it. We set everything up for you. This cost is INCLUDED in the price.' },
      { key: 'gallery',  label: '🖼 Photo gallery',           sub: 'Showcase your work or products',          price: 40,
        explain: 'A visual section on your site where you can display photos of your work, products, or portfolio. Looks great and helps customers see what you offer.' },
      { key: 'social',   label: '📱 Social media links',      sub: 'Instagram, Facebook, TikTok etc.',        price: 20,
        explain: 'Links to all your social media pages so visitors can follow you easily from your website.' },
      { key: 'whatsapp', label: '💬 WhatsApp chat button',    sub: 'Customers tap to message you directly',   price: 30,
        explain: 'A floating WhatsApp button on your site. Visitors tap it and it opens a WhatsApp chat directly to your number. Very popular with customers.' },
    ],
    extras: [
      { key: 'payment',  label: '💳 Accept payments',         sub: 'Requires backend — moves to Intermediate', price: 130,
        explain: 'To accept payments online, your site needs a backend (a server) to process transactions securely. Adding this moves your project to the Intermediate tier.' },
      { key: 'auth',     label: '🔐 User accounts (login)',   sub: 'Requires backend — moves to Intermediate', price: 120,
        explain: 'Letting users create accounts and log in requires a backend to store their data securely. This feature is available in the Intermediate or Complex tier.' },
      { key: 'admin',    label: '🗂 Admin dashboard',         sub: 'Requires backend — moves to Intermediate', price: 130,
        explain: 'An admin area where you can manage content, orders, or users requires a backend. Available in Intermediate or Complex tier.' },
    ],
  },
  intermediate: {
    included: [
    ...
    addons: [
      { key: 'payment',  label: '💳 Payment integration',    sub: 'Accept card, bank, mobile money',          price: 130,
        explain: 'Let customers pay directly on your website. You create a free Stripe account, add us as developer, and we connect everything. Your money goes straight to you. Stripe charges ~1.5–3% per transaction separately.' },
      { key: 'auth',     label: '🔐 User accounts',          sub: 'Customers can sign up and log in',         price: 120,
        explain: 'Customers create their own account — like signing up for an app. Once logged in, they can see their order history, saved details, and more.' },
      { key: 'booking',  label: '📅 Booking / appointment',  sub: 'Customers book sessions or slots',         price: 100,
        explain: 'A calendar or booking system where customers can pick a date and time for a service or appointment. Great for consultants, clinics, trainers.' },
      { key: 'seo',      label: '🔍 Google visibility (SEO)', sub: 'Help people find you on Google',          price: 80,
        explain: 'SEO setup makes sure Google understands what your site is about so it shows your website to the right people in search results.' },
      { key: 'domain',   label: '🏷 Domain + hosting setup', sub: 'yourname.com — SSL secured',               price: 50,
        explain: 'Your website address and hosting setup — fully included in your quote.' },
    ],
    extras: [
      { key: 'storage',  label: '☁️ File / photo upload',    sub: 'Users or you can upload media',            price: 80,
        explain: 'Lets people upload photos, documents, or files to your website. For example, customers uploading proof of payment or you uploading product images from your phone.' },
      { key: 'realtime', label: '⚡ Live / real-time updates', sub: 'Chat, live notifications',               price: 150,
        explain: 'Things update instantly without refreshing — like a live chat, real-time order status, or live notifications. More advanced and adds to cost.' },
    ],
  },
  complex: {
    included: [
    ...
    addons: [
      { key: 'realtime', label: '⚡ Live / real-time features', sub: 'Chat, live notifications, updates',     price: 150,
        explain: 'Things update instantly without refreshing the page — like a live chat, real-time order tracking, or instant notifications.' },
      { key: 'multi_lang', label: '🌍 Multiple languages',     sub: 'Site in 2 or more languages',            price: 120,
        explain: 'Your website in more than one language — for example English and French. Great if you serve customers in different countries.' },
      { key: 'seo',      label: '🔍 Advanced SEO',             sub: 'Deep search optimisation',               price: 100,
        explain: 'Advanced SEO work beyond basics — structured data, page speed, keyword strategy, and monthly reporting.' },
      { key: 'domain',   label: '🏷 Domain + hosting setup',   sub: 'yourname.com — SSL secured',             price: 50,
        explain: 'Full domain and hosting setup included in your quote.' },
    ],
    extras: [],
  },
};
...
const StuckHelper = ({ onClose }) => (
  <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4" style={{ background: 'rgba(0,0,0,0.6)' }} onClick={onClose}>
    <div className="bg-white rounded-3xl p-7 max-w-sm w-full shadow-2xl" onClick={e => e.stopPropagation()}>
      ...
      <p className="text-gray-600 text-sm leading-relaxed mb-5">Totally fine to feel unsure — websites can be confusing! Let us walk you through everything personally. Choose how you'd like to reach us:</p>
      <div className="flex flex-col gap-3">
        ...
        <a href="tel:+233501657205"
          className="flex items-center gap-3 p-4 rounded-2xl bg-purple-50 border border-purple-200 hover:bg-purple-100 transition-all">
          <span className="text-2xl">📞</span>
...
export default function WebsiteCalc() {
  ...
  const buildQuote = () => {
    ...
    return [
      ...
      `💡 30% deposit to start — pay by bank, card, or mobile money (Payoneer invoice, no account needed)`,
      ``,
  ...
  /* Stuck button — shown on every step */
  const StuckBtn = () => (
      ...
      <div className="max-w-xl mx-auto px-4 py-8">
        ...
        {step === 0 && (
          <div>
            ...
            <p className="text-gray-500 text-sm mb-6">Pick the one that fits best — this helps us give you the right estimate.</p>
        ...
        {step === 1 && (
          <div>
            ...
            <p className="text-gray-500 text-sm mb-6">Read each option carefully. The description tells you exactly who each one is for.</p>
            <div className="flex flex-col gap-5">
              {TIERS.map(t => (
                <button key={t.key} onClick={() => { setTier(t.key); setAddons([]); setExtras([]); }}
                  ...
                  {t.note && (
                    <div className="mt-3 text-xs text-gray-400 italic">{t.note}</div>
                  )}
                </button>
              ))}
            </div>
        ...
        {step === 2 && tier && (
          <div>
            ...
            <p className="text-gray-500 text-sm mb-1">Everything in your tier is already included. Add extras below to fit your exact needs.</p>
            <div className="flex items-center gap-2 mb-6 bg-green-50 border border-green-100 rounded-xl px-4 py-2">
              ...
              <span className="text-green-500 text-xs">· all add-ons stay within this</span>
            </div>

            {/* Included (non-toggleable) */}
            <div className="mb-5">
              <div className="text-xs font-bold uppercase tracking-widest text-gray-400 mb-3">Already included in your tier</div>
                ...
                {tierFeatures.included.map(f => (
                  <div key={f.key} className="flex items-center gap-3 p-3 rounded-xl bg-green-50 border border-green-100">
                    ...
                    <div className="flex-1">
                      <div className="font-bold text-sm text-green-800">{f.label}</div>
                      <div className="text-xs text-green-600">{f.sub}</div>
                    </div>
            ...
            {tierFeatures.addons?.length > 0 && (
              <div className="mb-5">
                <div className="text-xs font-bold uppercase tracking-widest text-gray-400 mb-3">Optional add-ons — still within {fmt(tierData.range[0])}–{fmt(tierData.range[1])} range</div>
                <div className="flex flex-col gap-3">
            ...
            {tierFeatures.extras?.length > 0 && (
              <div className="mb-5">
                <div className="flex items-center gap-2 mb-3">
                  <div className="text-xs font-bold uppercase tracking-widest text-orange-500">⚠️ Extras — these go beyond the {fmt(tierData.range[1])} range</div>
                </div>
                <div className="p-3 bg-orange-50 border border-orange-100 rounded-xl mb-3">
                  <p className="text-xs text-orange-700 leading-relaxed">Adding these will increase your cost beyond the stated range. That's okay — just be aware. You can still choose them if you need them.</p>
                </div>
                <div className="flex flex-col gap-3">
            ...
            <div className="bg-black text-white rounded-2xl p-4 flex items-center justify-between">
              <div>
                ...
                <div className="text-xl font-extrabold">{fmt(baseMin + addons.reduce((s,k) => { const f = tierFeatures.addons?.find(x=>x.key===k); return s+(f?.price||0);},0))} – {fmt(baseMax + addons.reduce((s,k) => { const f = tierFeatures.addons?.find(x=>x.key===k); return s+(f?.price||0);},0) + extrasSum)}</div>
              </div>
              {extras.length > 0 && (
        ...
        {step === 3 && (
          <div>
            ...
            <p className="text-gray-500 text-sm mb-6">If you can wait, you save money. Faster delivery costs more because we prioritise your project over others.</p>
                ...
                <button key={t.key} onClick={() => setTimeline(t.key)}
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`text-sm font-extrabold px-3 py-1 rounded-full
                      ${t.color === 'green' ? 'bg-green-100 text-green-700' : t.color === 'red' ? 'bg-red-100 text-red-600' : 'bg-gray-100 text-gray-600'}`}>
                      {t.badge}
                    </span>
                  </div>
                </button>
            ...
            <div className="p-4 bg-yellow-50 rounded-2xl border border-yellow-100 mb-1">
              <p className="text-sm font-bold text-yellow-800 mb-1">📅 Free progress meeting — always included</p>
              <p className="text-xs text-yellow-700 leading-relaxed">We show you what has been built before finishing. Free of charge. We just need you available when we schedule it.</p>
            </div>
            <StuckBtn />
          </div>
        )}
        ...
        {step === 4 && (
          <div>
            ...
            <p className="text-gray-500 text-sm mb-6">This is a starting range. We confirm the exact price together on a free call — no pressure, no commitment.</p>
            ...
            <div className="border border-gray-100 rounded-2xl overflow-hidden mb-5">
              ...
              {addons.map(k => {
                const f = tierFeatures.addons?.find(x => x.key === k);
                return f ? (
              ...
              {extras.map(k => {
                const f = tierFeatures.extras?.find(x => x.key === k);
                return f ? (
                  <div key={k} className="px-5 py-3 flex justify-between border-t border-gray-100 text-sm">
              ...
              <div className="px-5 py-3 border-t border-gray-100 bg-yellow-50">
                <p className="text-xs text-yellow-700"><span className="font-bold">💡 Professional email</span> (you@yourname.com) costs a little extra — or we guide you to set it up free yourself.</p>
              </div>
            </div>
            ...
            <div className="bg-gray-50 rounded-2xl p-5 mb-5">
              ...
              <div className="flex flex-col gap-2">
                {[
                  '30% deposit to start — sent via Payoneer invoice',
                  'No Payoneer account needed — pay by bank, card, or mobile money. No extra charges.',
                  'Remaining balance paid when your site is ready and you approve it',
                ].map((t, i) => (
            ...
            <div className="flex flex-col gap-3">
              <a href="https://calendly.com/blackbird77ad/free-consultation" target="_blank" rel="noopener noreferrer"
                className="w-full py-4 rounded-2xl bg-red-600 text-white font-extrabold text-base text-center block hover:bg-red-700 transition-all">
                📅 Book a Free Call to Confirm Pricing
              </a>
              <a href={`https://wa.me/233501657205?text=${encodeURIComponent(buildQuote())}`} target="_blank" rel="noopener noreferrer"
              ...
              <button onClick={() => { setStep(0); setIndustry(''); setTier(''); setAddons([]); setExtras([]); setTimeline('standard'); }}
            ...
            <p className="text-center text-gray-400 text-xs mt-5 leading-relaxed">
              Estimate only. Final price confirmed on your free call. No commitment required.
            </p>
